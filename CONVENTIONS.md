...
- **Batch all mark operations (removal + application) into a single transaction** — dispatch one `tr` with all `removeMark()` and `addMark()` calls rather than separate dispatches. This is safe because `removeMark` does not alter document structure or positions. Avoids multiple re-renders and keeps the operation atomic. Guard dispatch with a `hasChanges` flag to skip empty transactions.
- **Pre-build text-to-position mapping for phrase lookups** — `buildTextMapping()` walks the document once and produces both the searchable text string and a position map. All phrase lookups in a single call reuse this mapping instead of calling `doc.textBetween()` + `doc.descendants()` per flag. Reduces doc walks from 2N to 1 per call.
